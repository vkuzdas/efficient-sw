...
#  define wr_lock(lock) pthread_rwlock_rdlock(lock)
...
void esw_list_init(LIST_TYPE *list)
{
    ...
    list->head = NULL;
#elif defined (USE_RCU)
    CDS_INIT_LIST_HEAD(list); /* list.h Initialize a new list head. */
#else
#error "No lock type defined"
#endif
}
...
void esw_list_push(LIST_TYPE *list, const char *const key, const char *const value)
{
    ...
    wr_unlock(&list->lock);
#elif defined (USE_RCU)
    cds_list_add_rcu(&node->next, list); /* rculist.h Add new element at the head of the list. */ /// decreased R number
#endif
}
...
void esw_list_update(LIST_TYPE *list, const char *const key, const char *const value)
{
    ...
    wr_unlock(&list->lock);
#elif defined (USE_RCU)
    // https://github.com/urcu/userspace-rcu/blob/master/doc/examples/list/cds_list_replace_rcu.c
    esw_node_t *node, *n;
    cds_list_for_each_entry_safe(node, n, list, next) {
        if (strcmp(node->key, key) == 0) {
            free(node->value);
            char *node_value = (char *)calloc(sizeof(char), strlen(value) + 1);
            strcpy(node_value, value);
            node->value = node_value;
            node->checksum = calc_checksum(node_value);
        }
    }
#endif
}
...
#elif defined (USE_RCU)
    esw_node_t *current, *n;
    cds_list_for_each_entry_safe(current, n, list, next) {
        if (strcmp(current->key, key) == 0) {
            if (strlen(current->value) < max_len) {
                strcpy(value, current->value);
                if(calc_checksum(value) != current->checksum) err(1, "%s:%d wrong checksum", __FILE__, __LINE__);
